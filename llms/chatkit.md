...
- Omit `session_id` to search session titles, agent IDs and display names, and message bodies across the workspace.
...
Queries must contain 1 to 256 non-whitespace characters. Search is case-insensitive full-text matching, not fuzzy or substring matching. A session scope outside the selected organization and workspace returns `404` without revealing whether it exists elsewhere.

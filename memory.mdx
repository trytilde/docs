...
The synthesis agent receives bank-bound `recall`, `retain`, and document-deletion operations under `/memory/synthesis-sessions/{session_id}`. Clearing the synthesizer stops background processing but preserves queued evidence for a later assignment.

<Info>
  The automatic-memory and synthesis APIs are available. OpenBot's opt-in automatic-memory product wiring is still pending its dependent product stack; existing OpenBot agents do not gain automatic recall merely by upgrading the API.
</Info>
...
An uncertain provider outcome must be reconciled before another chargeable retry. The billing API primitives are available, but OpenBot hosted-inference metering is not part of the merged durable-work runtime yet.

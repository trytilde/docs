...
Tilde memory banks use a managed Helix graph and vector store. Tenant and bank predicates isolate every node, edge, search, and mutation. Recall combines semantic, text, exact-title, and graph candidates before returning bounded results with evidence and source provenance. Owners can also import and export portable bank templates for supported configuration, mental models, and directives.
...
Before Memory Catcher creates its synthesis AgentRun, reserves credits, or calls the selected provider, it submits the prompt's batch digest, complete evidence IDs, and worker lease to `/memory/synthesis-sessions/{session_id}/validate-batch`. Tilde accepts only the deterministic current oldest-first prompt chunk under that unexpired lease. Later or arbitrary same-lease subsets remain invalid until the earlier chunk completes.

...
OpenBot now applies this lifecycle to every model call made through Tilde-managed Vercel project OIDC. It writes AgentRun effect intent before the provider starts, records the Gateway generation for recovery, and settles the authoritative receipt before reporting hosted cost to the run. Direct owner Gateway keys and Codex subscription inference are outside this managed meter.

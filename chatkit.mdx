...
Pass the same session context to `context.session.createMCPClient()` when the agent consumes tools through MCP. Session tools are added to that client without asking the model for routing identifiers.

...
Tilde supplies child job ID, generation, model, and budget in the signed request’s typed `execution` context. Hidden continuations carry the run ID, worker ID, generation, and hidden flag there. Agent hosts must not accept caller-provided model, budget, run, or worker headers as authority. Use `chatKitAgentRunIdempotencyKey(triggerId, context.execution)` when creating a run so duplicate job deliveries reuse it and a resumed job generation can start a fresh run.

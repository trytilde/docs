...
Connect Codex, Claude Code, or Cursor with `openbot plugin` to record coding
...
a Tilde plugin because Codex hooks are plugin-owned; Claude Code and Cursor use
their user hook settings. Authentication stays in the existing Tilde plugin
token store, while the hook routing file contains only the API URL, team ID,
and agent ID.

...
Each Mission Control WebSocket retains its authenticated effective user and current Identity groups. Team agent lifecycle changes are sent to every connected team member. Private agent lifecycle changes are sent to users and groups admitted by its visibility plane. Private-session message, streaming, task, and agent-turn events use the session's current visibility grants and conversation membership. Internal audience identifiers are not included in client event payloads.
...
curl --get \
  ...
  "https://api.trytilde.ai/api/v1/team/$TILDE_TEAM_ID/chatkit/mission-control/search"
```

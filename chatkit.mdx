...
Signal provider instances may also be personal. A personal provider may feed only a private Routine created by the same owner. Sessions created by that Routine are `user_team` sessions for the owner, preventing personal activity from becoming a team-visible conversation.

Signal providers and Routines use the same two planes. Provider visibility controls discovery and delivery inspection. Routine visibility controls who can discover and run the configured action. Ownership controls configuration, target/session policy, grants, state-changing retries, and deletion. Deliveries and sessions inherit their provider and matched Routine authorization rather than defining independent grants.
...
A Routine is one authorization and lifecycle root for work that starts on a schedule or from a Signal. Each Routine contains 1–8 independently enabled triggers and records durable manual, scheduled, and event executions.

- A schedule trigger uses six-field UTC cron syntax.
- An event trigger selects a configured signal provider, signal type, optional JSON filters, session policy, and ChatKit action.
- Stable trigger IDs preserve schedule telemetry and retry progress when you replace a Routine.
- Updates use the returned `version` as `expected_version` so concurrent edits fail instead of overwriting each other.

Open **ChatKit** → **Routines** to create scheduled work. From a configured Signal, choose **Add event Routine** to select the event type, target agent, and stable session-key template.

The REST compatibility path remains `/api/v1/team/{team_id}/automations`, but its payload is the native Routine resource. Signal rules and the former `/chatkit/routines` API are no longer separate resources.

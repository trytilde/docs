...
ChatKit sessions and routines are either shared `team` resources or private `user_team` resources. A private session remains in its execution team but has one owner; that owner and team, organization, or system administrators can manage it. Messages, attachments, events, tasks, replies, and turn queues inherit the session's scope.

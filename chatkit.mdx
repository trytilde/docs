...
Credential values, approval tokens, OAuth state, and generated signing keys are never included in model-visible proposal text. If execution needs provider setup, the proposal returns a secret-free setup-item reference. The approving human continues through the normal server-authored credential or OAuth card. The original agent can then resume its task from the durable decision and proposal status.

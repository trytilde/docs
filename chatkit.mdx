...
Use consolidated ChatKit search to find session titles, agents participating in sessions, and message content across a workspace. Results include the session context needed to open the matching conversation.
...
To search messages inside one conversation, add its `session_id`. Session-scoped search returns message results only and returns `404` if the session does not belong to the selected workspace.
